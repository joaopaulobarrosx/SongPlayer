...
public final class SongsViewModel {
    ...
    public func loadMoreIfNeeded(currentSong: Song) {
    ...
    public func loadRecentlyPlayed(modelContext: ModelContext) {
    ...
    public func markAsPlayed(song: Song, modelContext: ModelContext) {
    ...
    public func removeRecentlyPlayed(song: Song, modelContext: ModelContext) {
        let trackId = song.trackId
        let descriptor = FetchDescriptor<CachedSong>(
            predicate: #Predicate { $0.trackId == trackId }
        )
    ...
    public func cacheSongs(_ songs: [Song], modelContext: ModelContext) {
        for song in songs {
            let trackId = song.trackId
            let descriptor = FetchDescriptor<CachedSong>(
                predicate: #Predicate { $0.trackId == trackId }
